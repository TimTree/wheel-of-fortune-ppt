...
Private Sub savePuzzle()
    ...
    If thereWasAPuzzle = True Then
        For j = 1 To 52
            ActivePresentation.Slides(11 + PuzzleIndex).Shapes("PuzzleSolution" & CStr(i + (12 * PuzzleIndex)) & "-" & CStr(j)).TextFrame.TextRange.Text = ActivePresentation.Slides(8).Shapes("BoardTile" & CStr(j)).OLEFormat.Object.Value
            ActivePresentation.Slides(11 + PuzzleIndex).Shapes("PuzzleSolution" & CStr(i + (12 * PuzzleIndex)) & "-" & CStr(j)).Fill.ForeColor.RGB = ActivePresentation.Slides(8).Shapes("BoardTile" & CStr(j)).OLEFormat.Object.BackColor
        Next j
        ActivePresentation.Slides(11 + PuzzleIndex).Shapes("PuzzleCategory" & CStr(i + (12 * PuzzleIndex))).TextFrame.TextRange.Text = ActivePresentation.Slides(8).Shapes("CategoryBox").OLEFormat.Object.Value
    End If
End Sub
...
Private Sub placePuzzleToSetUp(i As Integer)
    ...
    PuzzleIndex = CInt(ActivePresentation.Slides(7).Shapes("CurrentPuzzleRowIndex").TextFrame.TextRange.Text)
    For n = 1 To 52
        ActivePresentation.Slides(8).Shapes("BoardTile" & CStr(n)).OLEFormat.Object.Value = ActivePresentation.Slides(11 + PuzzleIndex).Shapes("PuzzleSolution" & CStr(i) & "-" & CStr(n)).TextFrame.TextRange.Text
    Next n
    ActivePresentation.Slides(8).Shapes("CategoryBox").OLEFormat.Object.Value = ActivePresentation.Slides(11 + PuzzleIndex).Shapes("PuzzleCategory" & CStr(i)).TextFrame.TextRange.Text
End Sub
...
Private Sub loadPuzzle(i As Integer)
    ...
    For j = 1 To 52
        ...
        If ActivePresentation.Slides(11 + PuzzleNumberIndex).Shapes("PuzzleSolution" & i & "-" & j).TextFrame.TextRange.Text <> "" Then
            ...
            End If
            If ActivePresentation.Slides(11 + PuzzleNumberIndex).Shapes("PuzzleSolution" & i & "-" & j).TextFrame.TextRange.Text = "Ñ" And _
            ActivePresentation.Slides(9).Shapes("SpanishÑ").TextFrame.TextRange.Text = "off" Then
               SpanishNError = True
               toggleSpanishN
            End If
        End If
    ...
    For k = 1 To 27
        If k < 27 Or (k = 27 And ActivePresentation.Slides(9).Shapes("SpanishÑ").TextFrame.TextRange.Text = "on") Then
    ...
    If SpanishNError Then
        MsgBox "This puzzle uses the letter Ñ, but the Spanish Ñ setting was disabled. This setting has automatically been enabled." & vbNewLine & vbNewLine & _
        "You can re-disable the Spanish Ñ in the Settings slide.", 0, "Spanish Ñ Note"
    End If
    Exit Sub
errHandler:
    Exit Sub
...
Private Function isLetter(strValue As String) As Boolean
    ...
    extendedChars = Array("Á", "À", "Â", "Ä", "Ç", "É", "È", "Ê", "Ë", "Í", "Ì", "Î", "Ï", "Ñ", "Ó", "Ò", "Ô", "Ö", "Ú", "Ù", "Û", "Ü")
...
Private Function lettersMatch(letter1 As String, letterSelectorLetter As String) As Boolean
    ...
    Select Case letterSelectorLetter
        Case "A"
            extendedChars = Array("A", "Á", "À", "Â", "Ä")
        Case "C"
            extendedChars = Array("C", "Ç")
        Case "E"
            extendedChars = Array("E", "É", "È", "Ê", "Ë")
        Case "I"
            extendedChars = Array("I", "Í", "Ì", "Î", "Ï")
        Case "O"
            extendedChars = Array("O", "Ó", "Ò", "Ô", "Ö")
        Case "U"
            extendedChars = Array("U", "Ú", "Ù", "Û", "Ü")
        Case Else
...
Private Sub bringLetterBack(i As Integer)
    If i = 27 Then
        ActivePresentation.Slides(2).Shapes("Letter" & i).TextFrame.TextRange.Text = "Ñ"
    Else:
...
Sub toggleSpanishN()
    If ActivePresentation.Slides(9).Shapes("SpanishÑ").TextFrame.TextRange.Text = "on" Then
        ActivePresentation.Slides(9).Shapes("SpanishÑ").TextFrame.TextRange.Text = "off"
        ActivePresentation.Slides(9).Shapes("SpanishÑ").Fill.ForeColor.RGB = RGB(217, 150, 148)
    ...
    Else:
        ActivePresentation.Slides(9).Shapes("SpanishÑ").TextFrame.TextRange.Text = "on"
        ActivePresentation.Slides(9).Shapes("SpanishÑ").Fill.ForeColor.RGB = RGB(195, 214, 155)
...
Sub ExplainSpanishN()
    MsgBox "If your puzzles are in Spanish, enable this setting to make Ñ a selectable letter.", 0, "Spanish Ñ Setting"
End Sub
