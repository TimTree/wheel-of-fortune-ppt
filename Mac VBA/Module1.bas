...
Sub PlayerRoundDollarSign(oSh As Shape)
    ...
    Next i
    If j = True Then
        Set roundDollarAmount = ActivePresentation.Slides(2).Shapes("Player" & i & "RoundDollarAmount")
        ...
        If IsNumeric(roundDollarAmount.TextFrame.TextRange.Text) = False Then
            MsgBox ("Please remove non-numeric characters from " + playerName.TextFrame.TextRange.Text + "'s score before using this action.")
        Else:
            addMoney = InputBox("Add money to " + playerName.TextFrame.TextRange.Text + "'s score." & vbNewLine & vbNewLine & "Enter the number of letters, multiplied by the wheel value.", "Quick Add", ActivePresentation.Slides(2).Shapes("LetterCounter").TextFrame.TextRange.Text)
            ...
            ElseIf IsNumeric(addMoney) = False Then
                ...
                If k = True Then
                    ...
                    Else:
                        ...
                        resultMoney = (splitMoney(0) + 0) / (splitMoney(1) + 0)
                    End If
                    roundDollarAmount.TextFrame.TextRange.Text = CDbl(roundDollarAmount.TextFrame.TextRange.Text) + (resultMoney + 0)
                Else:
                MsgBox ("You can only enter numbers or arithmetic expressions here.")
                End If
        Else:
            roundDollarAmount.TextFrame.TextRange.Text = CDbl(roundDollarAmount.TextFrame.TextRange.Text) + (addMoney + 0)
        End If
    End If
End If
End Sub
...
' Mac only
Sub editPlayerRoundScore(oClickedShape As Shape)
  Dim oSh As Shape
    Dim sText As String
    ...
    Next
    For i = 1 To 3
        If ActivePresentation.Slides(2).Shapes("Player" & i & "RoundDollarAmount").Name = oSh.Name Then
            j = True
            Exit For
        End If
    Next i
    If j = True Then
        sText = InputBox("Edit " & ActivePresentation.Slides(2).Shapes("Player" & i & "Name").TextFrame.TextRange.Text & "'s round score:", "Edit Player Round Score", oSh.TextFrame.TextRange.Text)
        While IsNumeric(sText) = False And sText <> ""
        sText = InputBox("You can only enter numbers here. Try again:", "Edit Player Round Score", sText)
        Wend
        If sText = "" Then
        Else:
...
Sub editPlayerTotalsScore(oClickedShape As Shape)
  Dim oSh As Shape
    Dim sText As String
    For Each oSh In SlideShowWindows(1).View.Slide.Shapes
        If oSh.Name = oClickedShape.Name Then
    ...
    Next
    For i = 1 To 3
        If ActivePresentation.Slides(2).Shapes("Player" & i & "TotalsDollarAmount").Name = oSh.Name Then
            j = True
            Exit For
        End If
    Next i
    If j = True Then
        sText = InputBox("Edit " & ActivePresentation.Slides(2).Shapes("Player" & i & "Name").TextFrame.TextRange.Text & "'s totals score:", "Edit Player Totals Score", oSh.TextFrame.TextRange.Text)
        While IsNumeric(sText) = False And sText <> ""
        sText = InputBox("You can only enter numbers here. Try again:", "Edit Player Totals Score", sText)
        Wend
        If sText = "" Then
        Else:
        oSh.TextFrame.TextRange.Text = sText
...
Sub PlayerBuyaVowel(oSh As Shape)
    ...
    If j = True Then
        ...
        Set playerName = ActivePresentation.Slides(2).Shapes("Player" & i & "Name")
        If IsNumeric(roundDollarAmount.TextFrame.TextRange.Text) = False Then
            MsgBox ("Please remove non-numeric characters from " + playerName.TextFrame.TextRange.Text + "'s score before buying a vowel.")
        ElseIf CDbl(roundDollarAmount.TextFrame.TextRange.Text) < VOWELCOST Then
            MsgBox (playerName.TextFrame.TextRange.Text + " cannot buy a vowel. Vowels cost $" + CStr(VOWELCOST) + ".")
        ElseIf CDbl(roundDollarAmount.TextFrame.TextRange.Text) >= VOWELCOST Then
            roundDollarAmount.TextFrame.TextRange.Text = (CDbl(roundDollarAmount.TextFrame.TextRange.Text) - VOWELCOST)
        End If
    End If
End Sub
...
Sub PlayerTransferTotals(oSh As Shape)
    ...
    If j = True Then
        Set roundDollarAmount = ActivePresentation.Slides(2).Shapes("Player" & i & "RoundDollarAmount")
        Set TotalsDollarAmount = ActivePresentation.Slides(2).Shapes("Player" & i & "TotalsDollarAmount")
        If IsNumeric(TotalsDollarAmount.TextFrame.TextRange.Text) = True And IsNumeric(roundDollarAmount.TextFrame.TextRange.Text) = True Then
            If CDbl(roundDollarAmount.TextFrame.TextRange.Text) < HOUSEMINIMUM Then
                shouldIHouse = MsgBox("The house minimum of $" + CStr(HOUSEMINIMUM) + " will be transferred.", vbOKCancel)
                If shouldIHouse = vbOK Then
                    TotalsDollarAmount.TextFrame.TextRange.Text = CDbl(TotalsDollarAmount.TextFrame.TextRange.Text) + HOUSEMINIMUM
                    wipeRoundScores
                Else:
                    Exit Sub
                End If
            Else:
                TotalsDollarAmount.TextFrame.TextRange.Text = CDbl(TotalsDollarAmount.TextFrame.TextRange.Text) + CDbl(roundDollarAmount.TextFrame.TextRange.Text)
                wipeRoundScores
            End If
        End If
    End If
...
Private Sub savePuzzle()
    ...
    If thereWasAPuzzle = True Then
        For j = 1 To 52
            ActivePresentation.Slides(10).Shapes("PuzzleSolution" & CStr(i) & "-" & CStr(j)).TextFrame.TextRange.Text = ActivePresentation.Slides(8).Shapes("SetUpPuzzle" & CStr(j)).TextFrame.TextRange.Text
            ActivePresentation.Slides(10).Shapes("PuzzleSolution" & CStr(i) & "-" & CStr(j)).Fill.ForeColor.RGB = ActivePresentation.Slides(8).Shapes("SetUpPuzzle" & CStr(j)).Fill.ForeColor.RGB
        Next j
        ActivePresentation.Slides(10).Shapes("PuzzleCategory" & CStr(i)).TextFrame.TextRange.Text = ActivePresentation.Slides(8).Shapes("SetUpPuzzleCategory").TextFrame.TextRange.Text
    End If
End Sub

Private Sub placePuzzleToSetUp(i)
    For n = 1 To 52
        ActivePresentation.Slides(8).Shapes("SetUpPuzzle" & CStr(n)).TextFrame.TextRange.Text = ActivePresentation.Slides(10).Shapes("PuzzleSolution" & CStr(i) & "-" & CStr(n)).TextFrame.TextRange.Text
        ActivePresentation.Slides(8).Shapes("SetUpPuzzle" & CStr(n)).Fill.ForeColor.RGB = ActivePresentation.Slides(10).Shapes("PuzzleSolution" & CStr(i) & "-" & CStr(n)).Fill.ForeColor.RGB
    Next n
    ActivePresentation.Slides(8).Shapes("SetUpPuzzleCategory").TextFrame.TextRange.Text = ActivePresentation.Slides(10).Shapes("PuzzleCategory" & CStr(i)).TextFrame.TextRange.Text
End Sub
...
Sub wipeOnClose()
    ...
    For i = 1 To 3
        ...
        ActivePresentation.Slides(2).Shapes("Player" & i & "TotalsDollarAmount").TextFrame.TextRange.Text = "0"
    Next i
...
Private Sub wipeRoundScores()
    Dim i As Integer
    For i = 1 To 3
        ActivePresentation.Slides(2).Shapes("Player" & i & "RoundDollarAmount").TextFrame.TextRange.Text = "0"
    Next i
End Sub
